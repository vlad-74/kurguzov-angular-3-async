...
export class Rx {
  ...
  func_Observable(destroyed) {
    ...
    obs.pipe(takeUntil(destroyed)).subscribe(
  ...
  func_Subject(destroyed) {
    ...
    sbj
      .pipe(takeUntil(destroyed))
      .subscribe(vl => console.log(`Subject - 1st: ${vl}`));
    sbj.next(3);
    sbj
      .pipe(takeUntil(destroyed))
      .subscribe(vl => console.log(`Subject - 2nd: ${vl}`));
    sbj.next(9);
  ...
  func_BehaviorSubject(destroyed) {
    ...
    Bsbj.pipe(takeUntil(destroyed)).subscribe(vl =>
      console.log(`BehaviorSubject - 1st: ${vl}`)
    );
    Bsbj.pipe(takeUntil(destroyed)).subscribe(vl =>
      console.log(`BehaviorSubject - 2nd: ${vl}`)
    );
    Bsbj.next(7);
  ...
  func_combineLatest(destroyed) {
    combineLatest(of(3, 6, 9), interval(500).pipe(take(3)))
      .pipe(takeUntil(destroyed))
  ...
  func_throwError(destroyed) {
    interval(1000)
      .pipe(
        mergeMap(x =>
          x === 2 ? throwError('Twos are bad') : of('a', 'b', 'c')
        )
      )
      .pipe(takeUntil(destroyed))
  ...
  func_retry(destroyed) {
    interval(1000)
      ...
      .subscribe(
        x => console.log(x),
        e => console.error(`${e}: Retried 2 times then quit!`)
      );
